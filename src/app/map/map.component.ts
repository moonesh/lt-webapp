  ...
  ngOnInit() {
    this.vehicleService.subscription.subscribe(vehicle => {
     ...
     this.vehicleService.centerVehicleHistory.subscribe(newHistory => {
       ...
       var vehicleWithAllLatLongs: Vehicle;
       vehicleWithAllLatLongs = <Vehicle> newHistory; 
    
     console.log(vehicleWithAllLatLongs);
     
     this.selectedVehicleHistory = polyline(vehicleWithAllLatLongs.historyofLatLong, {weight:10, opacity:0.5, color:'red'}).addTo(this.map);
